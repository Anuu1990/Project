...
# open file
comsumption_data = pd.read_csv('My_Electricity_Consumption.csv', delimiter=';')
...
comsumption_data['Time'] = pd.to_datetime(
    comsumption_data['Time'], format='%d.%m.%Y %H:%M', errors='coerce')
new_price_data['Time'] = pd.to_datetime(
    new_price_data['Time'], format='%d/%m/%Y %H.%M', errors='coerce').dt.round('h')
...
comsumption_data['Energy(kWh)'] = comsumption_data['Energy(kWh)'].str.replace(
    ',', '.').astype(float)

new_price_data['Price (cent/kWh)'] = new_price_data['Price (cent/kWh)'].str.replace(
    ',', '.').astype(float)

comsumption_data['Temperature'] = comsumption_data['Temperature'].str.replace(
    ',', '.').astype(float)
